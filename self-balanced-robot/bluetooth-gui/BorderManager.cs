...
namespace pinghengxiaoche {
    public class ControlBorderManager {
            ...
            )
        {
            ...
            form.ResizeBegin += new EventHandler((_sender, _e) => {          //防止边框拖出一片“轨迹”出来
                Resizing = true;
                form.Refresh();
            });
            form.ResizeEnd += new EventHandler((_sender, _e) => {            //防止边框拖出一片“轨迹”出来
                Resizing = false;
                form.Refresh();
                });
        ...
        private void DrawBorder(int id)                                 //绘制边框，有过渡效果。
        {
            ...
            timer.Tick += new EventHandler((_sender, _e) =>
            {
                ...
                if (n >= FrameCount || ControlGroups[id].state != CurrentState) {         //如果已经画完或者状态已经改变
                    timer.Stop();
                    timer.Dispose();
                    return;
                }
                ...
                graphics.DrawRectangle(new Pen(b), BorderRect);             //绘制矩形边框
